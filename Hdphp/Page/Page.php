...
class Page
{
    ...
    //文字描述
    protected $desc = array('pre' => '上一页', 'next' => '下一页', 'first' => '首页', 'end' => '尾页', 'unit' => '条');
    ...
    private function setUrl()
    {
        ...
        {
            if ( ! in_array($k, array('m', 'c', 'a', 'page')))
            {
                $url .= "$k=$v&";
            }
        }
        ...
        {
            $url = empty($url) ? '&' : '&' . $url;
            $url = "?m={$_GET['m']}&c={$_GET['c']}&a={$_GET['a']}{$url}page={page}";
        }
        else
        {
            //路由访问模式
            $url = Route::url() . "?{$url}page={page}";
        }
        $this->url = $url;
    }
     ...
     * @return [type]       [description]
     */
    public function desc(array $desc = array())
    {
        $this->desc = $desc;
    ...
    public function pre()
    {
        ...
        return "<li class='disabled'><a href='javascript:;'>{$this->desc['pre']}</a></li>";
    }
    ...
    public function next()
    {
        ...
        return "<li class='disabled'><a href='javascript:;'>{$next}</a></li>";
    }
