...
class Mysql
{
    ...
    private function getReadLink()
    {
        if ( ! self::$readLink)
        {
            $dns = 'mysql:host=' . Config::get('database.read.host') . ';dbname=' . Config::get('database.read.database') . ';charset=utf8';
    ...
    private function getWriteLink()
    {
        if ( ! self::$writeLink)
        {
            $dns = 'mysql:host=' . Config::get('database.write.host') . ';dbname=' . Config::get('database.write.database') . ';charset=utf8';
    ...
    public function getTableField($table, $full = false)
    {
        ...
        //字段缓存
        if ( ! DEBUG && F($name, false, 'storage/cache/field'))
        {
            $data = F($name, false, 'storage/cache/field');
        }
        else
        {
