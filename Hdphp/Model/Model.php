...
class Model implements ArrayAccess
{
     ...
     */
    protected function cacheTableInfo()
    {
        //缓存名
        $name = C('database.database') . '_' . $this->table;

        if (DEBUG === true || ! $field = Cache::dir('storage/cache/table')->get($name))
        {
            $field['field'] = array_keys($this->db->getTableField($this->table));

            $field['_pk'] = $this->db->getTablePrimaryKey($this->table);

            Cache::dir('storage/cache/table')->set($name, $field);
        }
    ...
    private function autoValidate($data, $type)
    {
        ...
        foreach ($this->validate as $validate)
        {
            ...
            foreach ($actions as $action)
            {
                ...
                {
                    if ($VaAction->$method($field, $value, $params) !== true)
                    {
                        $_SESSION['_validate'] = $this->error = $error;
